...
| **短 SHA（H5 修正提交）** | `7e81d7f` |
| **短 SHA（含本通知文档，推荐拉取）** | `ca2b6a9` |
| **说明** | `7e81d7f` 起含 `h5_universal.html` 本轮修正与 `scripts/validate_bundles.mjs`；`ca2b6a9` 在同内容基础上增加本 **`RELEASE_NOTICE_*.md`**，便于仓库内对照。二者 **H5 与 bundle 文件一致**，Agent1 任取其一检出均可部署同一版页面。 |

**OpenClaw / 维护人：** 若团队约定以 **`main`**（或私库其他分支）为唯一部署源，请先将上述提交 **合并/推送** 到该分支，并在群内 **更新**「源分支 + 短 SHA」；Agent1 以 **最终约定分支上的 HEAD** 为准拉取。

**阻塞时：** 可按既有流程提供 **`cursor-out`** 包或指定 cherry-pick **`7e81d7f`**（或检出 **`ca2b6a9`**），确保现网拿到的 `h5_universal.html` 与校验脚本版本一致。
...
短 SHA：ca2b6a9（推荐，含发布通知）；H5 与 7e81d7f 等价
