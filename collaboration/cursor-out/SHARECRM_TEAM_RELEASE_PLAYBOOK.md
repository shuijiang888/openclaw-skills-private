# 纷享销客 ShareCRM · 第三轮发布团队协作单（OpenClaw / 小江 → Agent1）
...
```text
OpenClaw / 小江：把 open 仓库 feature/sharecrm-channel-ops 的 sharecrm 树同步到「部署用私库」并 push
        ↓
   （宿主机五条 grep 全 OK）
        ↓
...
2. **建议对齐分支 tip：** `feature/sharecrm-channel-ops` **当前 HEAD**（须含本文件 `SHARECRM_TEAM_RELEASE_PLAYBOOK.md` 与提交 **`4987b0a`** 起的 `server.mjs` / `web` 变更）。
3. 同步范围（相对仓库根）至少包含：
   - `collaboration/sharecrm_channel_ops/api/server.mjs`
   - `collaboration/sharecrm_channel_ops/web/`（`index.html`、`app.js`、`styles.css`、**`value-map-snippet.html`**）
   - `collaboration/sharecrm_channel_ops/README.md`（可选但建议）
   - `collaboration/sharecrm_channel_ops/AGENT1_任务单_仅部署.md`（已含 `scenarios/playbook` 自检）
...
详细命令见：**`collaboration/sharecrm_channel_ops/AGENT1_任务单_仅部署.md`**（第 2～3 步）。

要点复述：

- **构建 context** 必须是 **`collaboration/sharecrm_channel_ops/`**（勿仅用 `api/`）。
- 建议：`docker compose down` → `build --no-cache` → `up -d --force-recreate`。

### 3.3 本轮新增验收（在以往 health / scorecard / batches 之外）

将 **`BASE`** 换成公网实际前缀（子路径示例 `http://IP/sharecrm`）：
...
# 应含 demo_mode（布尔）；生产高管演示可设环境变量 SHARECRM_DEMO_MODE=1
...
7. **（可选）** 容器环境 `SHARECRM_DEMO_MODE=1`：侧栏无「数据与录入」、无绩效 JSON 调试块、无总览技术说明。
...
| 文档 | 作用 |
|------|------|
| 本文 **`SHARECRM_TEAM_RELEASE_PLAYBOOK.md`** | 本轮 **团队协作主单**（OpenClaw/小江 + Agent1） |
| `OPENCLAW_REQUEST_SHARECRM_SYNC_AND_DEPLOY.md` | 首轮 OpenClaw 同步诉求（含 `b27fdfc` 背景），仍可作流程参考 |
...
**文档版本：** v2 · 含高管演示（价值图谱 / 简报复制 / SHARECRM_DEMO_MODE）  
**维护：** Cursor · `feature/sharecrm-channel-ops`
...
```text
请按 collaboration/cursor-out/SHARECRM_TEAM_RELEASE_PLAYBOOK.md 第二节执行：
把 open 仓库 feature/sharecrm-channel-ops 当前 HEAD（须含 4987b0a、高管演示相关提交如 ba98871、及 SHARECRM_TEAM_RELEASE_PLAYBOOK.md）下的 collaboration/sharecrm_channel_ops/ 与本文档同步到部署私库并 push；
...
```text
请等私库同步完成（五条 grep 已 OK）后，按 collaboration/sharecrm_channel_ops/AGENT1_任务单_仅部署.md 构建部署；
并验收 SHARECRM_TEAM_RELEASE_PLAYBOOK.md 第三节与 3.4（含 value-map-html、价值图谱页、复制高管简报）。回传该文档里的 Agent1 回传模板。
```

**发群 / 汇总一条：**

```text
纷享销客 ShareCRM 第三轮发布协作：先 OpenClaw/小江同步 open→私库（见 SHARECRM_TEAM_RELEASE_PLAYBOOK.md §2），再 Agent1 按任务单 + §3 验收；真源分支 feature/sharecrm-channel-ops。
```
