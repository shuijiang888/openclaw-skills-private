...
2. **建议对齐分支 tip：** `feature/srne-channel-ops` **当前 HEAD**（写本单时不低于 **`5766bcc`**，含本协作单；功能核心在 **`4987b0a`**）。
3. 同步范围（相对仓库根）至少包含：
...
把 open 仓库 feature/srne-channel-ops（tip 不低于 5766bcc，含 4987b0a）的 collaboration/srne_channel_ops/ 与 cursor-out/SRNE_TEAM_RELEASE_PLAYBOOK.md 同步到部署私库并 push；
