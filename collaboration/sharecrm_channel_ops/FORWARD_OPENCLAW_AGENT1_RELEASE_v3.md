...
1. **确认源：** Cursor 侧交付在仓库路径 `collaboration/sharecrm_channel_ops/`（含 `api/server.mjs`、`web/*`、`data/seed.json`、`README.md` 等）。
2. **同步到部署用仓库：** 将含本轮改动的提交 **合并/推送** 到 Agent1 实际 `pull` 的分支（建议与历史一致：`feature/sharecrm-channel-ops` 或团队约定主分支）。
...
5. **阻塞时：** 若私库与 open 不同步，按既有流程打 **`collaboration/cursor-out/sharecrm_channel_ops_*.tar.gz`** 或等价制品，并更新 `cursor-out/AGENT1_SHARECRM_INGEST_AND_DEPLOY.md` 中的文件名与校验说明。
...
**须一并发布的制品路径（与 v2 相同）：**  
`collaboration/sharecrm_channel_ops/` 整包；Docker **build context = 该目录**，勿仅用 `api/`。
...
grep -q '/v1/channels/.*/competitors' collaboration/sharecrm_channel_ops/api/server.mjs && echo "OK competitors route"
grep -q 'ch360Mount' collaboration/sharecrm_channel_ops/web/index.html && echo "OK ch360 DOM"
```

然后：`docker compose build --no-cache` → `up -d`（或等价流程）；确认 **`SHARECRM_DB_PATH` 卷挂载**、`JWT_SECRET` 已替换。

---

## 四、验收清单（请 Agent1 回传）

**接口（带 Bearer，替换 `BASE` 为实际前缀，如 `http://IP/sharecrm`）**
...
**浏览器（`admin@sharecrm.demo` / `Demo2026!`）**
