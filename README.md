...
当前 **`package.json`** 与页脚 **`v0.2.0`**、**`lib/app-release.ts`** 应对齐；发版打标示例：`git tag -a v0.2.0 -m "release 0.2.0"`。
...
1. 启动后看页脚是否为 **v0.2.0**。
2. **销售经理**：任选项目 → 改价/提交审批；侧栏为清单。
3. 切 **销售总监**：**项目列表** →「待我处理」；进工作台 → 助手 **解析** + **角色话术**追加 → 应用到系数。
4. 切 **总经理**：后台 **主数据·客户** 可读；导入区仅提示；页脚 **落地准备（后台）** 可点（经理档无此链接）。
5. 切 **管理员**：**智能体审计**；保存/解析成功后提示可点审计（非管理员为文字说明）。
