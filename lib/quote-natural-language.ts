...
export function parseQuoteNaturalLanguage(
  ...
  if (/战略|龙头|总部直签|集团客户|高价值客户/.test(t)) {
  ...
  if (/定制|非标|方案型|多轮改板|多方评审/.test(t)) {
    apply({ cP: cP + 0.07 }, "识别到定制复杂度：产品系数上调");
    hints.push("复杂交易通常不走「自动通道」，请以分流结论为准。");
  }
  if (/标品|目录料|成熟料号/.test(t)) {
    apply({ cP: cP - 0.03 }, "识别到标准品倾向：产品系数小幅下调");
  }

  if (/加急|赶工|硬节点|插单|锁交付|月底冲刺/.test(t)) {
  ...
  if (/小批量|试产|样品|打样|首单|试用订阅/.test(t)) {
    apply({ cQ: cQ + 0.06 }, "识别到小批量/打样：批量系数上调（小单议价空间）");
  }
  if (/百万级|大单|年度框|集中交付|年度订阅|多年签约/.test(t)) {
    apply({ cQ: cQ - 0.05 }, "识别到规模订单：批量系数小幅下调");
  }

  if (/出口|海外认证|报关|离岸|跨区域团队/.test(t)) {
    apply({ cR: cR + 0.05 }, "识别到海外场景：区域系数上调");
  }
  if (/内销|本地配套|园区直供|本地团队/.test(t)) {
    apply({ cR: cR - 0.03 }, "识别到内销/本地：区域系数小幅下调");
  }

  if (/壁垒高|专利|国产替代|稀缺产能|高复购|产品粘性/.test(t)) {
    apply({ cI: cI + 0.06 }, "识别到行业壁垒：行业系数上调");
  }
  if (/大宗|材料占比较高|行情波动|竞争激烈|低价替代/.test(t)) {
  ...
  if (summary.length === 0) {
    hints.push(
      "未匹配到明确规则。可尝试包含：高价值客户、赢单概率、月底冲刺、年度订阅、多方评审等关键词。",
    );
  }
